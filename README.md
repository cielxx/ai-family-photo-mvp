...
![高保真原型页面截图](assets/screenshots/2026-06-09_153004_239.png)
...
- `docs/TEST_REPORT.md`：v0.1 主流程回归测试记录。
...
1. 进入项目目录：
   ...
   ```bash
   cd ai-family-photo-mvp
   ```

   如果仓库被克隆到自定义文件夹，请进入实际的仓库根目录。
...
当前仓库包含一个原生微信小程序本地 mock 版，用于验证移动端主流程。它暂未接入云开发、Dify 或真实图像 API。
...
如果还没有正式小程序 AppID，可以先使用测试号或游客模式打开。第一版流程包括：首页、服务选择、上传与需求、生成中、结果展示和二次调整。
